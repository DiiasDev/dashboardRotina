...
export const fetchLembretes = async (): Promise<LembretesData[]> => {
  try {
    const response = await fetch(`${API_BASE_URL}/lembretes`);
    if (!response.ok) {
      throw new Error(`Erro ao buscar lembretes: ${response.status}`)
    }
    return await response.json();
  } catch (error) {
    console.warn("Erro ao buscar lembretes:", error)
    throw error
  }
}
...
export const createLembrete = async  (taskData: Omit<LembretesData, 'id'  | 'date'> & { date: string }): Promise<LembretesData> => {
  try {
    // Não precisa converter, já vem string ISO do frontend
    const payload = {
      ...taskData,
    };

    console.log('Enviando lembrete:', JSON.stringify(payload, null, 2));
    ...
    if (!response.ok) {
      throw new Error(`Erro ao criar lembrete: ${response.status}`);
    }
